...
describe('Composer', () => {
  it('disables Send until there is text, and sends the trimmed content on submit', async () => {
    ...
    await user.type(screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i), '  Hello there  ');
  ...
  it('sends on Enter and inserts a newline on Shift+Enter', async () => {
    ...
    const textarea = screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i);
  ...
  it('does not offer Retry after a cancelled turn — only a Dismiss action', () => {
    ...
    expect(screen.getByText(/đã hủy/i)).toBeInTheDocument();
  ...
  it('disables the textarea while sending or streaming', () => {
    ...
    expect(screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i)).toBeDisabled();
  ...
  describe('draft preservation (Sprint 2B audit Finding 3)', () => {
    it('never clears the draft itself on submit — clearing is entirely the parent’s responsibility', async () => {
      ...
      expect(screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i)).toHaveValue('Still here');
    });
    ...
    it('renders whatever draft text the parent currently holds, regardless of status', () => {
      ...
      expect(screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i)).toHaveValue('my unsent message');

      rerender(
        <Composer status="idle" errorMessage={null} draft="" onDraftChange={noop} onSend={noop} onCancel={noop} onRetry={noop} onDismiss={noop} />,
      );
      expect(screen.getByLabelText(/nhắn tin cho người bạn đồng hành/i)).toHaveValue('');
    });
  });
});
