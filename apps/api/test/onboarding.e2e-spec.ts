...
describe('Onboarding (e2e)', () => {
  ...
  it('persists conversation state across requests (refresh-safe draft)', async () => {
    const headers = await registerAndGetHeaders(app, uniqueEmail('draft'));

    const first = await request(app.getHttpServer()).get('/onboarding').set(headers).expect(200);
    ...
    const second = await request(app.getHttpServer()).get('/onboarding').set(headers).expect(200);
    expect(second.body.data.messages).toEqual(first.body.data.messages);
  });

  it('rejects a message sent in the wrong stage', async () => {
    const headers = await registerAndGetHeaders(app, uniqueEmail('stage-mismatch'));
  ...
  it('completes the full conversation, respects declined memory consent, and marks onboardingCompletedAt', async () => {
    ...
    const me = await request(app.getHttpServer()).get('/auth/me').set(headers).expect(200);
  ...
  it('creates a memory note only when the user accepts memory consent', async () => {
    const headers = await registerAndGetHeaders(app, uniqueEmail('consent-accept'));
